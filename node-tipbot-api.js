            ...
            case true:
                request(request_options)
                    ...
                    .pipe(es.mapSync(function(data) {
                        callback(data);
                        ((opts.verbose) ? console.log( 'streamed from '+ request_options.uri +' in: %ds', ( Date.now() - start ) / 1000 ) : '' );
                    }));
                break;
            case false:
                sendRequest()
                    .then(function(data) {
                        ...
                        ((opts.verbose) ? console.log('requested from ' + request_options.uri + ' in: %ds', (Date.now() - start) / 1000) : '');
                    })
